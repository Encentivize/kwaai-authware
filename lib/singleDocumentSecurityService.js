...
var multiTenantTools=function(options,mainCallback){
    ...
    if (!mainCallback){mainCallback=function(){}}

    function crudLoadCompleted(err){
        if (err){return mainCallback(err);}

        function crudWareLoadCompleted(err){
            if (err){return mainCallback(err);}
            return mainCallback(null);
        }
        kwaaiCrudware=require('kwaai-crudware').crudWare(connectionString,connectionOptions,crudWareLoadCompleted);
    }
    kwaaiCrud=require("kwaai-crud").crudTools(connectionString,connectionOptions,crudLoadCompleted);
