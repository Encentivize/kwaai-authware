...
var multiTenantTools={
    ...
    ,checkTenant:
        function(req,res,next){
            function setTenanttoReq(err,tenant){
                ...
                var tenantId=null;
                if (tenant._id instanceof mongo.ObjectID) { tenantId = tenant._id }
                else { tenantId = new mongo.ObjectID(tenant._id) }
    ...
    ,getUser:
        function(tenantName,userName,callback){
            function tenantRetrieved(err,tenant){

                if (err){return callback(err)}
                if (!tenant){return callback("Tenant not found")}
                var foundUser=null;
                var found = false;
            ...
            multiTenantTools.getTenantbyName(tenantName,tenantRetrieved)
        }
}
